...
NDP_predic <- survey_data %>%
  mutate(NDP_pref = case_when( survey_data$political_pref == "(3) NDP (New Democratic Party, New Democrats, NDPers)" ~ 1,
                              survey_data$political_pref != "(3) NDP (New Democratic Party, New Democrats, NDPers)" ~ 0))
...
model.1 <- lm(as.factor(supports_ALP) ~ gender + age_group,
            data = example_poll, family = binomial)
...
model.1 <- glmer( ~ X1 + X2 + (1 | Group), family = binomial)
