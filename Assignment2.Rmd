...
To make this estimate we are going to hypothesise and setup our model using the *survey data* and train it to estimate whether someone **will be voting for a particular party or not**. To do this we will be using **Multilevel Logistic Regression**, logistic regression is used when we are dealing with a **binary response variable**. In our case the result of running this model on our **CENSUS DATA** will generate **log odds** of the particular person voting in favour of our hypothesised party. \

Now in general the log odds generated from the regression model will look like:
 $$\log(\frac{p_{h_{0}}}{1-p_{h_{0}}}) = \beta_{0} + \beta_{1}x_{1} + \beta_{2}x_{2} + ... + \beta_{k}x_{k} \qquad i \subset [1,k]$$ 
Here $\beta{0}$ is the measure for the level is random intercept which accounts for the different income levels and $\beta_{i}, i \subset[1,k] $ are the other factors which effect our model. \
...
Now after generating the model and running the 
