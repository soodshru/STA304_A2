...
## make the according data sets according to the Support of their party
...
model.NDP <- svyglm( NDP_pref ~ as.factor(sex) + as.factor(education) + age + as.factor(income_family)+ as.factor(province), design.NDP, family="binomial")
#results
summary(model.NDP)
...
model.NDP.1 <- svyglm( NDP_pref ~ sex + education + age + income_family + province, design.NDP, family="binomial")
#results
summary(model.NDP.1)
...
#Final Models Prefrence **Uzi**
We need to predict results for the 3 biggest parties in Canada ie. NDP
```{r}
